...
public class EditVideoThread extends Thread {
    ...
    private WeakReference<EditMediaThread> mediaMuxer; // 音视频混合器
    ...
    public EditVideoThread(int mWidth, int mHeight, WeakReference<EditMediaThread> mediaMuxer) {
        // 初始化相关对象和参数
        this.mWidth = mWidth;
    ...
    private void encodeFrame(byte[] input) {
        ...
        do {
            if (outputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                EditMediaThread mediaMuxerRunnable = this.mediaMuxer.get();
                if (mediaMuxerRunnable != null) {
                    mediaMuxerRunnable.addTrackIndex(EditMediaThread.TRACK_VIDEO, newFormat);
                }
                ...
                if (mBufferInfo.size != 0) {
                    EditMediaThread mediaMuxer = this.mediaMuxer.get();

                    if (mediaMuxer != null && !mediaMuxer.isVideoTrackAdd()) {
                        MediaFormat newFormat = mMediaCodec.getOutputFormat();
                        mediaMuxer.addTrackIndex(EditMediaThread.TRACK_VIDEO, newFormat);
                    }
                    ...
                    if (mediaMuxer != null && mediaMuxer.isMuxerStart()) {
                        mediaMuxer.addMuxerData(new EditMediaThread.MuxerData(EditMediaThread.TRACK_VIDEO, outputBuffer, mBufferInfo));
                    }
