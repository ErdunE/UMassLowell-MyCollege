...
public class EditAudioThread extends Thread {
    ...
    private WeakReference<EditMediaThread> mediaMuxerRunnable;
    ...
    public EditAudioThread(WeakReference<EditMediaThread> mediaMuxerRunnable) {
    ...
    private void encode(final ByteBuffer buffer, final int length, final long presentationTimeUs) {
        ...
        /*获取解码后的数据*/
        final EditMediaThread muxer = mediaMuxerRunnable.get();
        if (muxer == null) {
        ...
        do {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                EditMediaThread mediaMuxerRunnable = this.mediaMuxerRunnable.get();
                if (mediaMuxerRunnable != null) {
                    Log.e(TAG, "添加音轨 INFO_OUTPUT_FORMAT_CHANGED " + format.toString());
                    mediaMuxerRunnable.addTrackIndex(EditMediaThread.TRACK_AUDIO, format);
                }
                ...
                if (mBufferInfo.size != 0 && muxer != null && muxer.isMuxerStart()) {
                    ...
                    muxer.addMuxerData(new EditMediaThread.MuxerData(EditMediaThread.TRACK_AUDIO, encodedData, mBufferInfo));
