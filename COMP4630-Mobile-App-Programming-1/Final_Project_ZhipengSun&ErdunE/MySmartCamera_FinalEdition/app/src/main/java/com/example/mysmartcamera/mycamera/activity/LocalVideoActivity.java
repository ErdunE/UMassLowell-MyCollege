...
import com.example.mysmartcamera.mycamera.adapter.MyAdapter;
import com.example.qoutelibyary.quote.voiceEdit.MediaPlayerWrapper;
import com.example.mysmartcamera.mycamera.views.ThumbnailView;
...
import static com.example.mysmartcamera.mycamera.utils.StaticFinalValues.ISSAVEVIDEOTEMPEXIST;
import static com.example.mysmartcamera.mycamera.utils.StaticFinalValues.VIDEOTEMP;
...
public class LocalVideoActivity extends AppCompatActivity implements MediaPlayerWrapper.IMediaCallback {
    ...
    private int rotate;
    public MyAdapter mThumbAdapter;
    ...
    private void initListener() {
        ...
        mLocalThumbView.setOnScrollBorderListener(new ThumbnailView.OnScrollBorderListener() {
            ...
            public void OnScrollBorder(float start, float end) {
                ...
                mStartTime = mVideoDuration * v;
                float right = mLocalThumbView.getRightInterval();
                mThumbSelTime = (int) ((right - left) * 30 / MyApp.screenWidth);
                float width = mLocalThumbView.getTotalWidth();
                if (right == width) {
                ...
                mLocalSelTimeTv.setText("Selected" + mThumbSelTime + "s");
...
//                cmd.append("ffmpeg");
    ...
    private void initThumbs() {
        ...
        if (mEndTime < 30) {
            mLocalSelTimeTv.setText("Selected" + mEndTime + "s");
        }
        final int frame;
        final int frameTime;
            ...
            if (localVideoActivity != null) {
                switch (msg.what) {
                    case CLIPPER_FAILURE:
                        Toast.makeText(localVideoActivity.mContext, "video compilation failure, please try another video", Toast.LENGTH_LONG).show();
                    case CLIPPER_GONE:
                    ...
                    case SUBMIT:
                        ...
                        localVideoActivity.mThumbAdapter.setLoadSuccessCallBack(new MyAdapter.LoadSuccessCallBack() {
                            @Override
                            public void callback() {
                                ...
                                if (i < MyApp.screenWidth) {
                                    if (i > MyApp.screenWidth / 6) {
                                        localVideoActivity.mLocalThumbView.setWidth(i + DisplayUtil.dp2px(localVideoActivity,1));
                                    } else {
                                        localVideoActivity.mLocalThumbView.setWidth(MyApp.screenWidth / 6 - DisplayUtil.dp2px(localVideoActivity,10));
                                    }
                                }
