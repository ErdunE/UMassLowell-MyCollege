package com.example.mysmartcamera.mycamera.threads;
...
import static com.example.mysmartcamera.mycamera.threads.EditVideoThread.IMAGE_HEIGHT;
import static com.example.mysmartcamera.mycamera.threads.EditVideoThread.IMAGE_WIDTH;


public class EditMediaThread extends Thread {
    ...
    private static EditMediaThread mediaMuxerThread;

    private EditAudioThread audioThread;
    private EditVideoThread videoThread;
    ...
    public static void startMuxer() {
        if (mediaMuxerThread == null) {
            synchronized (EditMediaThread.class) {
                if (mediaMuxerThread == null) {
                    mediaMuxerThread = new EditMediaThread();
    ...
    private void initMuxer() {
        ...
        audioThread = new EditAudioThread((new WeakReference<EditMediaThread>(this)));
        videoThread = new EditVideoThread(IMAGE_WIDTH, IMAGE_HEIGHT, new WeakReference<EditMediaThread>(this));
        audioThread.start();
        videoThread.start();
        try {
            readyStart();
